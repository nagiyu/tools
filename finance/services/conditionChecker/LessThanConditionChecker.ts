...
export class LessThanConditionChecker extends BaseConditionChecker {
  readonly conditionType = FINANCE_NOTIFICATION_CONDITION_TYPE.LESS_THAN;
  ...
  async check(params: ConditionCheckParams): Promise<Condition> {
    ...
    if (currentPrice < conditionValue) {
      return this.createCondition(true, `${target} price ${currentPrice} is less than ${conditionValue}`);
    }
