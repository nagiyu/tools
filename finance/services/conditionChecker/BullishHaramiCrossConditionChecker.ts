...
export class BullishHaramiCrossConditionChecker extends BaseConditionChecker {
  readonly conditionType = FINANCE_NOTIFICATION_CONDITION_TYPE.BULLISH_HARAMI_CROSS;
  ...
  async check(params: ConditionCheckParams): Promise<Condition> {
    ...
    try {
      ...
      if (firstIsBullish && firstIsLarge && secondIsBullish && secondContained && secondIsSmaller) {
        return this.createSuccessCondition(target, 'Bullish Harami - bullish reversal signal');
      }
